...
class EditarFilmeScreen extends StatefulWidget {
  ...
  const EditarFilmeScreen({super.key, required this.filme});

  @override
  State<EditarFilmeScreen> createState() => _EditarFilmeScreenState();
}

class _EditarFilmeScreenState extends State<EditarFilmeScreen> {
  final FilmeController _controller = FilmeController();

  late TextEditingController _imagem;
  ...
  void initState() {
    super.initState();
    _imagem = TextEditingController(text: widget.filme.imagem);
  ...
  Future<void> _confirmarEdicao() async {
    final bool? confirmar = await showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text("Deseja salvar as alterações deste filme?"),
        actions: [
          TextButton(
  ...
  Future<void> _salvarEdicao() async {
    ...
    var resultado = await _controller.editar(widget.filme.id!.toString(), filmeEditado);
